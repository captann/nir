...
PowerState* Diagnostics_GetPowerState(void) {
    return &diagState.powerState;
}
...
void ADC0SS1_Handler(void) {
    ...
    float voltage = (voltage_raw * VREF / ADC_MAX_VALUE) * VOLTAGE_DIVIDER;
...
void WDT0_Handler(void) {
    ...
    // Логируем событие перед возможным сбросом
    Diagnostics_Log(LOG_FATAL, DIAG_EVENT_WATCHDOG_RESET,
                   "Watchdog timeout", diagState.watchdogTimeout);
    
    // WDT автоматически сбросит систему, если не будет вовремя обновлен
}
...
typedef struct {
    TestStatus powerTest;
    TestStatus sensorTest;
    TestStatus motorTest;
    TestStatus commTest;
    TestStatus safetyTest;
    uint32_t timestamp;
    bool isTestComplete;
...
float Diagnostics_ConvertADCToTemp(ADCResult adcResult) {
    ...
    const float MIN_TEMP = -50.0f;  // Минимальная температур для термопар
    const float MAX_TEMP = 150.0f;  // Максимальная температур для термопар
...
uint32_t GetMotorTemperature(int motorIndex) {
    // Настройка канала АЦП для температур моторов
    ADC1_SSMUX1_R = ADC_AIN1 + motorIndex;  // Выбираем канал AIN1 с учетом индекса мотора
    ADC1_SSCTL1_R = 0x06;      // Установка флагов завершения и сравнения
    ADC1_SSFIFO1_R;            // Очистка FIFO
    ADC1_ISC_R = ADC_ISC_IN1;  // Сброс флага прерывания
    return ADC1_SSFIFO1_R;     // Возвращаем значение АЦП
...
float GetColdJunctionTemperature(void) {
    ...
    // Преобразование значения АЦП в температур
    float temperature = Diagnostics_ConvertADCToTemp(adcResult);
